...
int main() {
  FILE *ArchCajaDiaria;
  CajaDiaria cd;
  FormaPago formasPago[50], forma_pago_max_comisiones;
  int n, m, j, k, forma_pago_act, subtotal_ventas, tot_ventas, i, min_ventas;
  float tot_importe, tot_comisiones, max_comisiones, total = 0;
  char codigo_act[4], caja_min_ventas[4];
  Total totales[20];
  ...
  for (j = 0; j < n; j++) {
    totales[j].forma_pago = formasPago[j];
    totales[j].total = 0;
  }
